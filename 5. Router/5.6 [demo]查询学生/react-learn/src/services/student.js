...
export function getStudentsPageByKeywordAndSex({ page = 1, limit = 10, keyword = "", sex = "" } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open("GET", `http://localhost:3000/api/student/findByPage?page=${page}&limit=${limit}&keyword=${keyword}&sex=${sex}`, true);
    // 真实网络请求可以不需要加请求地址baseURL前缀，在package.json加proxy
    // xhr.open("GET", `/api/student/findByPage?page=${page}&limit=${limit}`, true);
    console.log(`http://localhost:3000/api/student/findByPage?page=${page}&limit=${limit}&keyword=${keyword}&sex=${sex}`);
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          try {
