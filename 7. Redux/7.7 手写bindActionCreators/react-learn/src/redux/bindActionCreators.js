...
export default function bindActionCreators(actionCreator, dispatch) {
  ...
  if (typeof actionCreator === "function") {
    return getActionAndDispatch(actionCreator, dispatch);
    ...
    for (const key in actionCreator) {
      ...
      result[key] = getActionAndDispatch(createAction, dispatch);
...
function getActionAndDispatch(actionCreator, dispatch) {
  return function (...args) {
    const action = actionCreator(...args);
    dispatch(action);
  };
